...
│   │   ├── items.js         # 상점 아이템 데이터
│   │   └── worlds.js        # 월드 설정 데이터 (스테이지별 기능/보스)
│   └── scenes/
│       └── SnakeGame.js     # 메인 게임 로직 (~19000 lines)
├── public/
│   └── assets/
...
- **gridSize**: 20px
- **UI 영역**: 상단 60px
- **속도**: 모든 스테이지 90ms 시작, 먹이당 5ms 감소, 최대 속도 50ms
...
게임은 월드 단위로 구성되며, 각 월드는 3개의 스테이지로 이루어짐 (마지막 스테이지가 보스전).
...
| 기능 | 해금 시점 | 설명 |
|------|----------|------|
| 십자가 후레쉬 | Stage 1-3 (World 0) | 1~5번째 먹이 위치 강조 |
| 파티클 효과 | 6번째 먹이부터 | 먹이 생성 시 하늘색 별 파티클 |
| 상점 | Stage 3 클리어 후 | 아이템 구매 가능 |
| 보스전 | Stage 3, 6, 9, 12, 15... | 각 월드 마지막 스테이지 |
| 데드존 | Stage 4-6 (World 1) | 9번째 먹이 시 생성 |
| 안개(Fog of War) | Stage 7-9 (World 2) | 시야 제한 |
| 대출 기능 | Stage 8 클리어 후 | NEW 뱃지와 함께 등장 |
| 움직이는 톱니 | Stage 10-12 (World 3) | 매 먹이마다 톱니 1개 생성 (최대 5개) |
| 원형 독가스 자기장 | Stage 13-15 (World 4) | 배틀로얄 스타일 맵 축소 |

### 개발자 테스트 모드 (KK)

게임 중 'K' 키를 빠르게 두 번 누르면 개발자 모드가 활성화됩니다.

#### 기능
- **스테이지 선택**: 원하는 스테이지로 즉시 이동
- **테스트 스테이지 토글**: -2, -1, 0 스테이지 활성화/비활성화 (T 키)
- **월드 정보 표시**: 각 스테이지의 월드명과 보스 여부 표시
...
2. 활성화된 테스트 스테이지는 localStorage에 저장
3. 테스트 스테이지 클리어 시:
...
### World 0: Basic (Stage 1-3)
| Stage | 특이사항 |
|-------|----------|
| 1 | 십자가 후레쉬 (1~5번째 먹이), 텔레포트 1회 |
| 2 | 십자가 후레쉬, 텔레포트 2회 |
| 3 | **보스: 독개구리** (1000점 보너스), 상점 해금 |

### World 1: Deadzone (Stage 4-6)
| Stage | 특이사항 |
|-------|----------|
| 4 | 9번째 먹이 시 데드존 생성 |
| 5 | 스테이지 시작 시 데드존 3개 추가 |
| 6 | **보스: 총잡이 (Bullet Hell)** |

### World 2: Darkness (Stage 7-9)
| Stage | 특이사항 |
|-------|----------|
| 7 | **안개(Fog of War)** - 시야 제한 |
| 8 | 대출 기능 해금 (NEW 뱃지) |
| 9 | **보스: 녹턴 (Nocturne)** |

### World 3: Machine Kingdom (Stage 10-12)
| Stage | 특이사항 |
|-------|----------|
